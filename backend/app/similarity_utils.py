...
def rank_segments(query, video_segments, device='cpu'):
    ...
    ranked_segments = sorted(zip(summary_similarities, video_segments), key=lambda x: x[0], reverse=True)
    ...
    ranked_segments = sorted(zip(headline_similarities, top_summary_segments), key=lambda x: x[0], reverse=True)
    ...
    ranked_segments = sorted(zip(gist_similarities, top_headline_segments), key=lambda x: x[0], reverse=True)
